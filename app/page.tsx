...
export default function Home() {
  ...
  const [verifierModel, setVerifierModel] = useState<string>(
    VERIFIER_OPTIONS[0]
  );
  ...
  const analyze = async () => {
    ...
    try {
      ...
      formData.append("verifierModel", verifierModel);
        ...
        >
          <header className="shrink-0 px-4 py-4 border-b border-zinc-200">
            ...
            <p className="mt-1 text-xs text-zinc-600">
              Upload .pdf or .docx for section scores and flags. Reviewer
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="text-xs text-zinc-600">
                ...
                <select
                  ...
                  disabled={loading}
                  className="mt-1 w-full rounded-md border border-zinc-300 bg-white px-2 py-1.5 text-sm text-zinc-800 disabled:bg-zinc-100"
                >
                  {EVALUATOR_OPTIONS.map((model) => (
              ...
              <div className="space-y-4 text-sm">
                ...
                {selectedResult.sectionScores && (
                  <div>
                    ...
                    {selectedResult.strictAllPass !== undefined && (
                      <p className="mt-1 text-xs text-zinc-600">
                        Strict (all sections pass):{" "}
                        <span className="font-medium text-zinc-900">
                          {selectedResult.strictAllPass ? "Yes" : "No"}
                        </span>
                        {selectedResult.classificationStrategy && (
                          <span className="text-zinc-500">
                            {" "}
                            (strategy: {selectedResult.classificationStrategy})
                          </span>
                        )}
                      </p>
                    )}
                    {(selectedResult.evaluatorModel ||
                      selectedResult.verifierModel) && (
                      <p className="mt-1 text-xs text-zinc-600">
                        Models:{" "}
                        {selectedResult.evaluatorModel && (
                          <span className="font-medium text-zinc-900">
                            eval={selectedResult.evaluatorModel}
                          </span>
                        )}
                        {selectedResult.evaluatorModel &&
                          selectedResult.verifierModel && <span>{", "}</span>}
                        {selectedResult.verifierModel && (
                          <span className="font-medium text-zinc-900">
                            verify={selectedResult.verifierModel}
                          </span>
                        )}
                      </p>
                    )}
                    {(selectedResult.weightedScore !== undefined ||
                      selectedResult.pipelineConsistent !== undefined ||
                      selectedResult.pipelineIterations !== undefined) && (
                      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-zinc-600">
                        {selectedResult.weightedScore !== undefined && (
                          <span>
                            Weighted score:{" "}
                            <span className="font-medium text-zinc-900">
                              {(selectedResult.weightedScore * 100).toFixed(1)}
                            </span>
                            {" / 100"}
                          </span>
                        )}
                        {selectedResult.pipelineConsistent !== undefined && (
                          <span>
                            Consistent:{" "}
                            <span className="font-medium text-zinc-900">
                              {selectedResult.pipelineConsistent ? "Yes" : "No"}
                            </span>
                          </span>
                        )}
                        {selectedResult.pipelineIterations !== undefined && (
                          <span>
                            Feedback iterations:{" "}
                            <span className="font-medium text-zinc-900">
                              {selectedResult.pipelineIterations}
                            </span>
                          </span>
                        )}
                      </div>
